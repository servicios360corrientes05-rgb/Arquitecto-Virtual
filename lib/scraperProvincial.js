...
async function scrapeProvincial(adrema, outputPath) {
    ...
    const esProduccion = process.env.NODE_ENV === 'production' || process.env.RAILWAY_ENVIRONMENT;

    const launchOptions = {
        headless: esProduccion ? 'new' : false, // Headless en servidor, visible en local
    ...
    if (!esProduccion && executablePath) {
        ...
        console.log("   > ☁️ Entorno producción: usando Chromium bundled en modo headless.");
    }
