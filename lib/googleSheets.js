...
export async function registrarClienteEnSheets(datos) {
    try {
        ...
        if (!serviceAccountAuth) {
            ...
            // Normalizar: reemplazar \n literales, y asegurar saltos de línea reales
            privateKey = privateKey.replace(/\\n/g, '\n');
            // Si Railway escapó las comillas del JSON, limpiarlas
            if (privateKey.startsWith('"') && privateKey.endsWith('"')) {
                privateKey = privateKey.slice(1, -1).replace(/\\n/g, '\n');
            }
            console.log('[GoogleSheets] Key starts with:', privateKey.substring(0, 30));
            serviceAccountAuth = new JWT({
